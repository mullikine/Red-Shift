...
Sub InitialPhysics()
   ...
   For i = 0 To StarCount - 1
      With Stars(i)
         ...
         .z = Int(Rnd * 2 * StarRadius) - StarRadius
         While Sqr(.x ^ 2 + .y ^ 2 + .z ^ 2) > StarRadius
            .x = Int(Rnd * 2 * StarRadius) - StarRadius
            .y = Int(Rnd * 2 * StarRadius) - StarRadius
            .z = Int(Rnd * 2 * StarRadius) - StarRadius
         Wend
...
Sub Draw()
   ...
   For i = 0 To StarCount - 1
      With ScreenStars(i)
         .x = ZoomX(Stars(i).x - Ships(You.Ship).x + ScreenDims.Width / 2)
         .y = ZoomY(-(Stars(i).y - Ships(You.Ship).y) + ScreenDims.Height / 2)
         .color = Stars(i).color
         .rhw = Stars(i).rhw
      End With
   Next i
...
Dim randArg As Single
   ...
   RandomMargin = 0 'Ships(You.Ship).maMod
